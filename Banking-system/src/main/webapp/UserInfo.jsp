<%@page import="Bank.Model.UserInfoModels"%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
  ...
  <body>
            ...
            <ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
              <li class="nav-item">
                <a class="nav-link active"
							aria-current="page" href="<%= request.getContextPath()%>/staffModify?action=home&mainUser=${mainUser}">Home</a>
              </li>
    ...
    <div class="container">
      ...
      <% UserInfoModels info= (UserInfoModels) request.getAttribute("mainUserInformation");
        	if(info != null){
        ...
        <p>
          <strong>USer ID: </strong> <%= info.getUserId() %>
        </p>
        <p>
          <strong>Name: </strong> <%= info.getfName() %> <%= info.getlName() %>
        </p>
        <p>
          <strong>Date of Birth: </strong> <%=info.getDob() %>
        </p>
        <p>
          <strong>Father's Name: </strong> <%= info.getFatherName() %>
        </p>
        
        <p>
          <strong>Address: </strong> <%= info.getAddresss()%>
        </p>
        <p>
          <strong>City: </strong> <%= info.getCity()%>
        </p>
        <p>
          <strong>District: </strong> <%= info.getDistrict() %>
        </p>
        <p>
          <strong>State: </strong> <%= info.getState() %>
        </p>
        <p>
          <strong>Pin Code: </strong> <%= info.getPincode() %>
        </p>

        <p>
          <strong>Aadhaar Number: </strong> <%= info.getAadgarNo() %>
        </p>
        <p>
            <strong>Account Number: </strong> <%= info.getAccountNo() %>
          </p>
        <p>
          <strong>Pan Number: </strong> <%= info.getPanNo() %>
        </p>
        <p>
          <strong>Email: </strong> <%= info.getEmail() %>
        </p>
        <p>
          <strong>Phone Number: </strong> <%= info.getPhoneNo() %>
        </p>
        <p>
          <strong>Emergency Number: </strong> <%= info.getEmergencyNo() %>
        </p>
        <%}else{
        	out.print("Something, Issue in Server....!");
        }%>
      </div>
